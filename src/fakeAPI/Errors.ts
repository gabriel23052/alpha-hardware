export type ErrorID = (typeof Errors)["errors"][number][0];
...
class Errors {
  ...
  private static map = new Map<ErrorID, ResponseError>(
    this.errors.map(([id, message]) => [id, { id, message }]),
  );

  public static get(id?: ErrorID | null): ResponseError {
    const error = this.map.get(id ? id : "GENERIC");
    if (!error) throw new Error(`Error not found: ${id}`);
    return error;
  }
}
